...
- **ABI compatibility with the hook.** Field names, struct ordering, the
  `reponseResolve` typo — all matched. Verified by
  [`test/integration/SdkCrossValidation.t.sol`](../test/integration/SdkCrossValidation.t.sol),
  which feeds SDK-generated bytes through the actually-compiled hook.
